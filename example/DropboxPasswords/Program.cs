        ...
        internal class TextUi : IUi
        {
            public string PerformOAuthLogin(string url, string redirectUrl)
            {
                ...
                return Task.Run(async () =>
                    {
                        using (var browser = await Puppeteer.LaunchAsync(new LaunchOptions { ExecutablePath = chromePath, Headless = false }))
            ...
            public IUi.Action AskForNextAction()
            {
                while (true)
                {
                    Util.WriteLine("You need to approve the enrollment on one of your devices. Please choose:", ConsoleColor.Green);
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                if (config.ContainsKey("recovery-words"))
                {
                    ...
                    {
                        Console.WriteLine(
                            "Exactly 12 words separated by a single space should be provided.\n" + "See config.yaml.example for reference."
                        );
                        return;
                    }
                }
