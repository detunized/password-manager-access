...
namespace PasswordManagerAccess.LastPass
{
    internal static class Client
    {
        ...
        )
        {
            ...
            var (session, rest) = await Login(lowerCaseUsername, password, clientInfo, ui, transport, cancellationToken).ConfigureAwait(false);
            try
            {
        ...
        )
        {
            ...
            var result = await DuoV1.AuthenticateAsync(host, signature, ui, rest.Transport, cancellationToken).ConfigureAwait(false);
            if (result == null)
        ...
        )
        {
            ...
            var result = await DuoV4.AuthenticateAsync(url, ui, rest.Transport, cancellationToken).ConfigureAwait(false);
            if (result == null)
                return new OobWithExtras(OobResult.Cancel);
