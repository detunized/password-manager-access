...
namespace PasswordManagerAccess.LastPass
{
    internal static class Client
    {
        ...
        )
        {
            ...
            var result = await DuoV1.AuthenticateAsync(host, signature, ui, rest.Transport, logger, cancellationToken).ConfigureAwait(false);
            if (result == null)
        ...
        )
        {
            ...
            var result = await DuoV4.AuthenticateAsync(url, ui, rest.Transport, logger, cancellationToken).ConfigureAwait(false);
            if (result == null)
                return new OobWithExtras(OobResult.Cancel);
