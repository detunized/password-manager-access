...
public static partial class Client
{
    ...
    )
    {
        ...
        var result = SecondFactorResult.Done(new Dictionary<string, string> { ["dshmac"] = Util.HashRememberMeToken(token, sessionKey.Id) }, true);
    ...
    internal static async Task<T> GetEncryptedJsonAsync<T>(string endpoint, AesKey sessionKey, RestClient rest, CancellationToken cancellationToken)
    {
    ...
    private static readonly SecondFactorKind[] SecondFactorPriorityOtherPlatforms = { SecondFactorKind.Duo, SecondFactorKind.GoogleAuthenticator };
}
