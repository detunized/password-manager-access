    ...
    internal sealed class TwofishManagedTransform : ICryptoTransform
    {
        ...
        private class TwofishImplementation : IDisposable
        {
            ...
            private static DWord F32(DWord x, DWord[] k32, int keyLen)
            {
                ...
                {
                    x =
                        MdsTable[0, P8x8[P_01, P8x8[P_02, x.B0] ^ k32[1].B0] ^ k32[0].B0]
            ...
            [SuppressMessage(
                ...
                MessageId = "Member",
                Justification = "Multidimensional array does not waste space."
            )]
            ...
            {
                return (x >> 2) ^ (((x & 0x02) > 0) ? MDS_GF_FDBK / 2 : 0) ^ (((x & 0x01) > 0) ? MDS_GF_FDBK / 4 : 0);
            }
