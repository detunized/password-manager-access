    ...
    public class BoshTest : TestBase
    {
        ...
        public void Connect_and_GetChanges_connect_to_server_and_return_items()
        {
            ...
            var response4 =
                ...
                + "</iq>";
            var response5 =
                "<iq type='result' xmlns='jabber:client' id='_session_auth_2'>" + "<session xmlns='urn:ietf:params:xml:ns:xmpp-session'/>" + "</iq>";
            var response6 =
        ...
        {
            var flow = new RestFlow().Post(GetFixture("large-vault-response-1", "xml")).Post(GetFixture("large-vault-response-2", "xml"));

            // No Connect call here. Skipped as it's irrelevant in testing.
            var items = new Bosh("http://bosh.test", UserJid, "password", new RestBoshTransport(flow)).GetChanges("command-name", "4213").ToArray();
        ...
        private static readonly Jid UserJid = new Jid("206a9e27-f96a-44d5-ac0d-84efe4f1835a", "39.ucp-ntfy.kaspersky-labs.com", "portalu3mh3hwy2kp");
    }
