    ...
    public class AesCcmTest
    {
        ...
        public void Encrypt_throws_on_too_short_iv(int ivLength)
        {
            ...
            Exceptions.AssertThrowsInternalError(
                () => AesCcm.Encrypt(key: key, plaintext: new byte[1], iv: new byte[ivLength], adata: new byte[0], tagLength: 8),
        ...
        public void Encrypt_throws_on_invalid_tag_length(int tagLength)
        {
            var key = new byte[16];
            Exceptions.AssertThrowsInternalError(
                () => AesCcm.Encrypt(key: key, plaintext: new byte[1], iv: new byte[16], adata: new byte[0], tagLength: tagLength),
        ...
        {
            Exceptions.AssertThrowsInternalError(() => AesCcm.Decrypt(key, ciphertext, iv, adata, tagLength), "CCM tag doesn't match");
        }
